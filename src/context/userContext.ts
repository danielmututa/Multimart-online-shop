...
export const useAuthStore = create<AuthState & AuthActions>((set) => ({
  ...
  login: async (email: string, password: string, role?: string, isAdmin?: boolean, username?: string, phone?: string) => {
    ...
    try {
      ...
      // ✅ FIX: Decode password in case it was URL-encoded (e.g. space became %20)
      let decodedPassword = password;
      try {
        decodedPassword = decodeURIComponent(password);
      } catch {
        // If decoding fails, use the original password as-is
      ...
      } else if (username || (email && !email.includes('@'))) {
        loginPayload.username = username || email;
      }
  ...
  // ============================================
  // REGISTER - client, client_admin, and agent
  // ============================================
