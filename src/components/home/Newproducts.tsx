...
const NewProducts = () => {
  ...
  const fetchProducts = async () => {
    try {
      // Fetch products - the API should return products with client_admin data including phone
      ...
      if (response.data && response.data.length > 0) {
        ...
        console.log("First product client_admin:", response.data[0].client_admin)
        
        // Check if client_admin has phone
        if (response.data[0].client_admin) {
          console.log("Client Admin Phone:", response.data[0].client_admin.phone)
        } else {
          console.warn("No client_admin data found in product")
        }
      }
  ...
  // Add this new function to fetch client_admin phone number
// Update this function to handle different API response formats
const fetchClientAdminPhone = async (clientAdminId: number): Promise<string | null> => {
  try {
    ...
    console.log("Client Admin User Response:", response.data);
    
    // Handle different response formats
    // Format 1: response.data.phone
    if (response.data?.phone) {
      return response.data.phone;
    }
    
    // Format 2: response.data.data.phone
    if (response.data?.data?.phone) {
      return response.data.data.phone;
    }
    
    // Format 3: response.data.user.phone
    if (response.data?.user?.phone) {
      return response.data.user.phone;
    }
    
    console.warn("Phone number not found in response:", response.data);
    return null;
  } catch (err) {
...
//                     {selectedProduct?.client_admin?.phone ? (
//   <div className="w-full pt-4 border-t">
//     <WhatsAppContact 
